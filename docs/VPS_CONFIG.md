...
- Latest published firmware: `0.2.28`.
...
Do **not** OTA a build that defaults/migrates MQTT to `:8883` until TLS is
actually up. `0.2.11` did that and left the device offline.
...
## Deferred: finish MQTT security and per-device provisioning at home

This should be finished when physical access to the ESP32 is available, so the
device can be recovered over USB/Serial if the MQTT TLS cutover leaves it
offline.

Recommended order:

1. Keep current lab device (`cof-test`) working.
2. Finish MQTT TLS/auth cutover on the VPS.
3. OTA the firmware and confirm the existing device reconnects on `8883`.
4. Only after that, implement per-device credential provisioning in the web UI.

### Target provisioning model

For production, each device should have its own MQTT identity:

```text
username = device_uid
password = random secret generated by backend
ACL      = devices/<device_uid>/#
```
...
are only a bootstrap/lab setup for the current test device. They are not the
final multi-device model.

### Planned implementation

When resuming this work, implement the following in one pass:

1. Add a per-device MQTT credential record in the backend/database.
2. Generate a random password automatically when creating a new device.
3. Create/update the Mosquitto password entry for that device.
4. Show the credentials once in the UI together with a ready-to-paste Serial
   command:

```text
mqtt mqtt.callonfail.com.ar 8883 <device_uid> <device_uid> <device-password>
```

5. Store the credentials in ESP32 Preferences over Serial during first install.
6. Keep OTA/config/status fully remote after that first provisioning step.

### Why defer this to home

- The first provisioning flow still depends on Serial/USB.
- If TLS/auth is misconfigured, the device may stop reconnecting over MQTT.
- Physical access makes recovery safe and fast.
